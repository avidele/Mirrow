...
private:
    ...
    class_type& emplace_class(std::string_view name, class_type::construct_params&& params);
...
global_registry& global_registry::variable(std::string_view name) {
    using value_type = typename mirror::util::variable_pointer_traits<ptr>::type;
    ...
    params.setter_ = [](const any* value){
        *ptr = value->cast<value_type>();
    };
    params.getter_ = []() -> any{
        return any(*ptr);
    };
    return set_context(&global_context_.emplace_variable(name, std::move(params)));
}

template <typename args_tuple, size_t... I>
auto get_arg_type_info(std::index_sequence<I...>) {
    return std::vector<const type_info*>{get_type_info<std::tuple_element_t<I, args_tuple>>()...};
}
...
global_registry& global_registry::function(std::string_view name) {
    using args_type = typename mirror::util::function_traits<decltype(ptr)>::args;
    using ret_type = typename mirror::util::function_traits<decltype(ptr)>::return_type;
    ...
    params.arg_types = get_arg_type_info<args_type>(std::make_index_sequence<args_size>{});
    params.caller_ = [](any* args, uint8_t size) -> any{
        auto args_tuple = cast_array_to_args_tuple<args_type>(args, std::make_index_sequence<args_size>{});
    ...
    return set_context(&global_context_.emplace_function(name, std::move(params)));
}
...
class_registery<C> registry::Class(std::string_view name) {
    ...
    if constexpr(std::is_default_constructible_v<C>) {
        params.default_object_creator = []() -> any {
            return any(C());
        };
    }
    ...
    if constexpr (std::is_default_constructible_v<C>)
    {
        ...
        constructor_params.stack_constructor_ = [](any* args, uint8_t size) -> any{
            return any(C());
        };
        constructor_params.heap_constructor_ = [](any* args, uint8_t size) -> any{
            return any(new C());
        };
...
template <typename C>
class_registery<C>::class_registery(class_type& clazz) : clazz(clazz), meta_registry<class_registery<C>>(&clazz) {}
...
class_registery<C>& class_registery<C>::constructor(std::string_view name) {
    ...
    params.stack_constructor_ = [](any* args, uint8_t size) -> any{
        auto args_tuple = cast_array_to_args_tuple<args_tuple_t>(args, std::make_index_sequence<args_size>{});
        return any(std::apply(C::C, args_tuple));
    };
    params.heap_constructor_ = [](any* args, uint8_t size) -> any{
        auto args_tuple = cast_array_to_args_tuple<args_tuple_t>(args, std::make_index_sequence<args_size>{});
        return any(new C(std::apply(C::C, args_tuple)));
    };
    return meta_registry<class_registery>::set_context(&clazz.emplace_constructor(name, std::move(params)));
}
...
class_registery<C>& class_registery<C>::static_variable(std::string_view name) {
   ...
   return meta_registry<class_registery>::set_context(&clazz.emplace_static_variable(name, std::move(params)));
}
...
class_registery<C>& class_registery<C>::static_function(std::string_view name) {
    ...
    return meta_registry<class_registery>::set_context(&clazz.emplace_static_function(name, std::move(params)));
}

template <typename C>
template <auto Ptr>
class_registery<C>& class_registery<C>::member_variable(std::string_view name) {
    using class_type = typename mirror::util::variable_pointer_traits<Ptr>::class_t;
    using value_type = typename mirror::util::variable_pointer_traits<Ptr>::type;
    ...
    return meta_registry<class_registery>::set_context(&clazz.emplace_member_variable(name, std::move(params)));
}
...
{
    using class_type = typename mirror::util::function_traits<decltype(Ptr)>::class_type;
    using args_type = typename mirror::util::function_traits<decltype(Ptr)>::args;
    using ret_type = typename mirror::util::function_traits<decltype(Ptr)>::return_type;
    ...
    params.caller_ = [](any* object, any* args, uint8_t size) -> any{
        auto args_tuple = cast_array_to_args_tuple<args_type>(args, std::make_index_sequence<args_size>{});
        if constexpr(std::is_void_v<ret_type>){
            std::apply(Ptr, std::tuple_cat(std::make_tuple(object->cast<class_type&>()), args_tuple));
            return {};
        } else {
            return any(std::apply(Ptr, std::tuple_cat(std::make_tuple(object->cast<class_type&>()), args_tuple)));
        }
    };
    return meta_registry<class_registery>::set_context(&clazz.emplace_member_function(name, std::move(params)));
}
}  // namespace mirror::dynamic